...
export class OC {
	...
	std::list<std::pair<uint64_t, std::pair<Functor, Functor>>> _objectMoves; // moveObject, accelerateObject
...
public:
	...
	void insertObject(std::shared_ptr<Base>& r) {
		...
		if (Moveable* x = dynamic_cast<Moveable*>(r.get())) {
			_objectMoves.push_back({ r->getID(), std::make_pair(Functor([x]() { x->moveObject(); }), Functor([x]() { x->accelerateObject(); })) });
		}
